...
fn open_instance_subfolder(
    ...
    let target_dir = instances_dir
        .join(sanitize::sanitize_name(&instance_name))
        .join(&subfolder);
...
fn open_instance_folder(
    ...
    let app_data_dir = get_app_data_dir(&app_handle, &state)?;
    let instances_dir = app_data_dir.join("instances");
    let instance_dir = instances_dir.join(sanitize::sanitize_name(&instance_name));
...
fn open_crash_reports_folder(
    ...
    let target_dir = if crash_reports_dir.exists() {
        ...
        instances_dir.join(sanitize::sanitize_name(&instance_name))
    };
...
#[tauri::command]
fn get_stronghold_password(state: State<'_, AppState>) -> Result<String, String> {
    let key = state.stronghold_key.lock()
...
// ─── NeoForge Installer Commands ─────────────────────────────────

#[tauri::command]
fn install_neoforge_loader(
    app_handle: AppHandle,
    state: State<'_, AppState>,
    mc_version: String,
    ...
    neoforge_installer::install_neoforge_background(
        app_handle,
        app_data_dir,
        mc_version,
        neoforge_version,
...
#[tauri::command]
fn read_neoforge_version_json(
    app_handle: AppHandle,
    state: State<'_, AppState>,
    version_id: String,
) -> Result<String, String> {
    let app_data_dir = get_app_data_dir(&app_handle, &state)?;
    neoforge_installer::read_neoforge_version_json(&app_data_dir, &version_id)
}
...
fn read_screenshot(
    ...
    game_data::read_screenshot_as_base64(&path)
}
...
#[tauri::command]
fn read_app_icon() -> Result<String, String> {
    // Try to find icon.png relative to the executable or current dir
    let paths = [
        // During development, icon is in src-tauri/icons/
    ...
    Err("App icon not found".to_string())
}

// ─── Utility ─────────────────────────────────────────────────────────

/// Load or generate a random Stronghold password.
/// On first run, generates a 32-byte random key and persists it to a file.
/// On subsequent runs, reads it from the file.
fn load_or_generate_stronghold_key() -> String {
    // Try to read from APPDATA/accounts/stronghold_key.bin
    let key_path = dirs_data_local_dir()
        .map(|d| d.join("com.anonlauncher.app").join("accounts").join("stronghold_key.bin"));
    ...
    let key: String = (0..32)
        .map(|_| {
            let hex_digit = rand::thread_rng().gen_range(0..16);
            std::char::from_digit(hex_digit, 16).unwrap()
        })
        .collect();
...
/// Get the local data directory for the app.
fn dirs_data_local_dir() -> Option<std::path::PathBuf> {
    ...
    #[cfg(target_os = "macos")]
    {
        std::env::var("HOME").ok().map(|h| std::path::Path::new(&h).join("Library").join("Application Support"))
    }
    #[cfg(target_os = "linux")]
    {
        std::env::var("XDG_DATA_HOME").ok()
            .or_else(|| std::env::var("HOME").ok().map(|h| std::path::Path::new(&h).join(".local").join("share")))
    }
...
pub fn run() {
    tauri::Builder::default()
        ...
        .manage(AppState {
            ...
            stronghold_key: Mutex::new(load_or_generate_stronghold_key()),
        })
