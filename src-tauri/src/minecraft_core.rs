...
impl MinecraftCore {
    ...
    fn download_library(&self, lib: &LibraryToDownload) -> Result<(), String> {
        ...
        // ── SHA1 verification ──────────────────────────────────────
        // Mojang provides SHA1 hashes for all libraries. Verify before writing.
        if !lib.sha1.is_empty() {
            let mut hasher = Sha1::new();
            hasher.update(&bytes);
            let actual_hash = hex::encode(hasher.finalize());
            if actual_hash != lib.sha1 {
                return Err(format!(
                    "SHA1 mismatch for library '{}': expected {}, got {}",
                    lib.path, lib.sha1, actual_hash
                ));
            }
        }
    ...
    pub fn download_libraries(
        ...
        for (i, lib) in libraries.iter().enumerate() {
            ...
            self.emit_progress(app_handle, DownloadProgressEvent {
                ...
                current: i,
                total,
                status: format!("Downloading libraries ({}/{})...", i, total),
            });
        ...
        self.emit_progress(app_handle, DownloadProgressEvent {
            ...
            current: total,
            total,
            status: format!("Downloading libraries ({}/{}) — done", total, total),
        });
    ...
    pub fn download_assets(
        ...
        self.emit_progress(app_handle, DownloadProgressEvent {
            ...
            current: 0,
            total: 1,
            status: "Downloading asset index...".to_string(),
        });
        ...
        self.emit_progress(app_handle, DownloadProgressEvent {
            ...
            current: 0,
            total: total_assets,
            status: format!("Checking {} assets...", total_assets),
        });
        ...
        self.emit_progress(app_handle, DownloadProgressEvent {
            ...
            total: total_assets,
            status:format!("Assets: {} already cached, {} to download",
        ...
        if need_to_download == 0 {
            self.emit_progress(app_handle, DownloadProgressEvent {
                ...
                total: total_assets,
                status: format!("Downloading assets — all already cached ({})", total_assets),
            });
            ...
            let reporter = scope.spawn(move || {
                loop {
                    ...
                    let _ = app_handle.emit("download:progress", DownloadProgressEvent {
                        ...
                        total,
                        status: format!("Downloading assets ({}/{})...", done, total),
                    });
                        ...
                        if let Ok(resp) = reqwest::blocking::get(&asset_url) {
                            if resp.status().is_success() {
                                if let Ok(bytes) = resp.bytes() {
                                    // ── SHA1 verification ──────────────
                                    // Asset filename IS the SHA1 hash
                                    let mut hasher = Sha1::new();
                                    hasher.update(&bytes);
                                    let actual_hash = hex::encode(hasher.finalize());
                                    if actual_hash != hash {
                                        continue; // Corrupt download, skip
                                    }
        ...
        self.emit_progress(app_handle, DownloadProgressEvent {
            ...
            current: total,
            total,
            status: format!("Downloading assets — done ({} assets)", total),
        });
    ...
    pub fn extract_natives(
        ...
        for (i, native) in natives.iter().enumerate() {
            self.emit_progress(app_handle, DownloadProgressEvent {
                ...
                current: i,
                total,
                status: format!("Extracting native libraries ({}/{})...", i, total),
            });
        ...
        self.emit_progress(app_handle, DownloadProgressEvent {
            ...
            current: total,
            total,
            status: "Extracting native libraries — done".to_string(),
        });
    ...
    pub fn download_client_jar(
        ...
        if !json_path.exists() {
            self.emit_progress(app_handle, DownloadProgressEvent {
                ...
                current: 0,
                total: 2,
                status: format!("Downloading {}.json...", mc_version),
            });
            ...
            if let Ok(metadata) = fs::metadata(&jar_path) {
                if metadata.len() == expected_size {
                    self.emit_progress(app_handle, DownloadProgressEvent {
                        ...
                        current: 2,
                        total: 2,
                        status: "Downloading client.jar — done".to_string(),
                    });
