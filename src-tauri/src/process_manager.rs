...
impl ProcessManager {
    ...
    pub fn launch(
        ...
        std::thread::spawn(move || {
            ...
            // Reader finished — don't emit instance:stopped here!
            // NeoForge/Forge use a launcher pattern: the initial Java process
            // may exit after spawning the real Minecraft JVM, closing stdout/stderr.
        ...
        let health_name = instance_name.to_string();
        let health_pid = pid;
        std::thread::spawn(move || {
            loop {
                std::thread::sleep(std::time::Duration::from_secs(5));
                if !is_process_alive(health_pid) {
                    let _ = health_handle.emit(
                        "instance:stopped",
                        serde_json::json!({
                            "instanceName": health_name,
                            "exitCode": null,
                        }),
                    );
                    break;
