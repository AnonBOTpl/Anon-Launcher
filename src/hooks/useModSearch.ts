...
export function useModSearch(initialFilters?: Partial<ModSearchFilters>) {
  ...
  // Sync loader when initialFilters.loader changes (e.g. ModpackSearch toggle)
  useEffect(() => {
    const loader = initialFilters?.loader;
    if (loader) {
      setFilters((prev) => ({ ...prev, loader }));
    }
  }, [initialFilters?.loader]);
  ...
  const doSearch = useCallback(
    async (searchFilters: ModSearchFilters, offset: number, append: boolean) => {
      ...
      try {
        ...
        // Filter by loader (only for mods and modpacks, not resourcepacks or shaders)
        if (searchFilters.loader && (searchFilters.projectType === "mod" || searchFilters.projectType === "modpack")) {
