...
export function generateLaunchArgs(
  ...
  // Auto-connect to server using --quickPlayMultiplayer (1.20.5+)
  // This takes a direct server address string, e.g. "mc.hypixel.net" or "mc.hypixel.net:25565"
  // Minecraft silently ignores unrecognized args on older versions, so it's safe.
  if (server) {
    const address = server.port
      ? `${server.ip}:${server.port}`
      : server.ip;
    finalGameArgs.push("--quickPlayMultiplayer", address);
  }
