 ...
 */
/**
 * Fetch NeoForge version JSON from Tauri backend.
 */
export async function fetchNeoForgeJson(
  versionId: string,
): Promise<MinecraftVersionJson> {
  const { invoke } = await import("@tauri-apps/api/core");
  const raw = await invoke<string>("read_neoforge_version_json", { versionId });
  return JSON.parse(raw);
}

export async function resolveVersion(
  mcVersion: string,
  loader?: { type: "fabric" | "neoforge"; version: string },
  ...
  // ── NeoForge: merge NeoForge-generated version JSON ──────────────
  // NeoForge installer creates: versions/neoforge-<ver>/neoforge-<ver>.json
  if (loader?.type === "neoforge" && loader.version) {
    const neoforgeVersionId = `neoforge-${loader.version}`;
    const neoforgeJson = await fetchNeoForgeJson(neoforgeVersionId);

    // NeoForge overrides main class
    mainClass = neoforgeJson.mainClass;

    // Add NeoForge libraries
    if (neoforgeJson.libraries) {
      libraries = [...libraries, ...neoforgeJson.libraries];
    }

    // Merge NeoForge arguments
    if (neoforgeJson.arguments) {
      if (neoforgeJson.arguments.game) {
        gameArgs = [
          ...(Array.isArray(gameArgs) ? gameArgs : []),
          ...neoforgeJson.arguments.game,
        ];
      }
      if (neoforgeJson.arguments.jvm) {
        jvmArgs = [
          ...(Array.isArray(jvmArgs) ? jvmArgs : []),
  ...
  const features: Record<string, boolean> = {
    ...
    // Enable quick play support so version JSON quick play arguments are included.
    // Minecraft 1.20.5+ requires --quickPlayPath <dir> for quick play to work.
    // On older versions these arguments don't exist, so enabling them is harmless.
    // The actual server connection (--quickPlayMultiplayer) is added manually
    // in generateLaunchArgs when a server is provided.
