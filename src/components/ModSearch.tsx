...
function ModDetails({ slug, instanceName, mcVersion, loader, onBack, isInstalled, onUpdated }: ModDetailsProps) {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      setError(null);
      try {
        const loaders = loader ? [loader] : ["fabric"];
  ...
  }, [selectedVersion, depState.deps, instanceName, mcVersion, loader, project, onUpdated]);
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex items-start gap-4">
        {project.icon_url ? (
          ...
          <div className="flex h-16 w-16 items-center justify-center rounded-xl bg-gradient-to-br from-primary/20 to-primary/5 ring-1 ring-border/50">
          ...
          <div className="flex items-center gap-3">
            ...
            {isInstalled ? (
              ...
              <Button
                ...
                className="shrink-0 bg-gradient-to-r from-primary to-primary/80 hover:from-primary/80 hover:to-primary text-white shadow-lg shadow-primary/20"
              >
      ...
      <div className="flex flex-wrap gap-2">
        <a
          ...
          className="inline-flex items-center gap-1.5 rounded-lg border border-border/50 px-3 py-1.5 text-xs text-primary hover:text-primary hover:border-primary/30 hover:bg-primary/5 transition-colors"
        >
...
function ModSearchResult({ hit, isInstalled, onSelect }: ModSearchResultProps) {
  const { t } = useTranslation();
  const mainLoader = hit.categories.includes("neoforge") ? "neoforge" : hit.categories.includes("fabric") ? "fabric" : null;

  return (
    <button
      onClick={() => onSelect(hit.slug)}
      className="flex w-full items-start gap-3 rounded-xl border border-border/50 bg-card/50 p-3 text-left transition-all hover:border-primary/30 hover:bg-primary/5 hover:shadow-sm hover:shadow-primary/5"
    >
      {hit.icon_url ? (
        <img
          src={hit.icon_url}
        ...
        <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-gradient-to-br from-primary/20 to-primary/5 ring-1 ring-border/50">
...
function FilterBar({ mcVersion, category, sort, loader, onMcVersionChange, onCategoryChange, onSortChange }: FilterBarProps) {
  const { t } = useTranslation();
  ...
  const loaderLabel = loader === "neoforge" ? "NeoForge" : "Fabric";
  const loaderClass = loader === "neoforge"
    ? "bg-sky-500/10 text-sky-400 ring-sky-500/20"
    : "bg-emerald-500/10 text-emerald-400 ring-emerald-500/20";

  return (
    <div className="flex flex-wrap items-center gap-2">
      <span className={cn("rounded-md px-2.5 py-1 text-xs font-medium ring-1", loaderClass)}>
        {loaderLabel}
      </span>
      {showMcInput ? (
        <div className="flex items-center gap-1">
          <input
            ...
            autoFocus
          />
          <button onClick={handleMcApply} className="rounded-md bg-primary px-2 py-1 text-[10px] text-white hover:bg-primary transition-colors">OK</button>
        </div>
      ) : (
        <button
          ...
          className={cn(
            ...
            mcVersion ? "bg-primary/10 text-primary ring-1 ring-primary/30" : "text-muted-foreground hover:text-foreground hover:bg-muted",
          )}
        >
...
function ModSearch({ instanceName, instanceMcVersion, instanceLoader, installedMods, initialQuery, onUpdated }: ModSearchProps) {
  const { t } = useTranslation();
  const searchLoader = instanceLoader && instanceLoader !== "vanilla" ? instanceLoader : "fabric";
  const {
    results,
    loading,
    error,
    ...
    setFilter,
    loadMore,
  } = useModSearch({ mcVersion: instanceMcVersion, loader: searchLoader });
