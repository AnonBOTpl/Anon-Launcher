...
function ModList({ instanceName, instanceMcVersion, instanceLoader, onUpdatesFound, disabled }: ModListProps) {
  const { t } = useTranslation();
  ...
  const abortUpdateAllRef = useRef(false);
  ...
  const handleConfirmUpdateAll = useCallback(
    async (snapshotMode: "full" | "metadata" | "skip") => {
      ...
      abortUpdateAllRef.current = false;
      setUpdateAllProgress({ current: 0, total, currentName: "" });

      for (let i = 0; i < updates.length; i++) {
        // Check if user cancelled
        if (abortUpdateAllRef.current) {
          break;
        }

        const update = updates[i]!;
        setUpdateAllProgress({ current: i + 1, total, currentName: update.mod.name || update.mod.fileName });
        try {
          await updateModApi(
            instanceName,
            update.mod.fileName,
  ...
  const handleCancelUpdateAll = useCallback(() => {
    abortUpdateAllRef.current = true;
  }, []);
  ...
  return (
    <div className="space-y-4">
      ...
      {updateAllProgress && (
        <div className="rounded-lg border border-amber-500/20 bg-amber-500/5 p-3">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[11px] text-amber-400/80 shrink-0 whitespace-nowrap">
              {updateAllProgress.currentName} ({updateAllProgress.current}/{updateAllProgress.total})
            </span>
            <button
              onClick={handleCancelUpdateAll}
              className="shrink-0 rounded bg-destructive/20 px-2 py-1 text-[10px] text-destructive hover:bg-destructive/30 transition-colors"
            >
              {t("mods.cancel")}
            </button>
          </div>
      ...
      {/* Snapshot dialog — portaled to body for correct overlay */}
      {snapshotBeforeUpdate && snapshotBeforeUpdate.mode === null && !snapshotBeforeUpdate.creating && createPortal(
        <div
      ...
      {/* Creating snapshot loading — portaled to body */}
      {snapshotBeforeUpdate && snapshotBeforeUpdate.creating && createPortal(
