  ...
  return (
    <div className="flex items-start gap-3 rounded-xl border border-border/50 bg-card/50 p-3 transition-all hover:border-purple-500/30 hover:bg-purple-500/5">
      ...
      {hit.icon_url ? (
        ...
        <div className="mt-0.5 flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-gradient-to-br from-purple-600/20 to-purple-600/5 ring-1 ring-border/50">
    ...
    <div className="space-y-4 animate-fade-in">
      ...
      <div className="flex items-start gap-4">
        {hit.icon_url ? (
          ...
          <div className="flex h-14 w-14 items-center justify-center rounded-xl bg-gradient-to-br from-purple-600/20 to-purple-600/5 ring-1 ring-border/50">
            <span className="text-xl font-bold text-purple-400">{hit.title.charAt(0)}</span>
          </div>
        )}
        <div className="flex-1 min-w-0">
...
function ModpackSearch({ onSelect }: ModpackSearchProps) {
  ...
  const {
    ...
    setFilter,
    loadMore,
  } = useModSearch({ projectType: "modpack", loader: "fabric" });
  ...
  // Load versions when a modpack is selected
  useEffect(() => {
    ...
    async function loadVersions() {
      try {
        // hit is non-null here (guarded by early return above)
        const vers = await getProjectVersions((hit!).slug, {
          loaders: ["fabric"],
        });
        if (!cancelled) {
          // Filter to only modpack-type versions
