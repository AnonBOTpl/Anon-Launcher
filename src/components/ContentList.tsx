...
interface ContentListProps {
  ...
  /** Instance loader type (to customize shader install guidance) */
  ...
  return (
    <div className="flex items-center gap-3 rounded-xl border border-border/50 bg-card/50 px-4 py-3 transition-all">
      ...
      {item.iconUrl ? (
        ...
        <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-gradient-to-br from-primary/20 to-primary/5 text-xs font-bold text-primary ring-1 ring-border/50">
...
function ContentList({ instanceName, folder, irisInstalled, onInstallIris, instanceLoader, disabled }: ContentListProps) {
  const { t } = useTranslation();
  ...
  return (
    <div className="space-y-4">
            ...
            className={cn(
              ...
              showSearch
                ...
                : disabled
                  ...
                  : "bg-gradient-to-r from-primary to-primary/80 hover:from-primary/80 hover:to-primary text-white shadow-lg shadow-primary/20",
            )}
          >
            {showSearch ? (
              <>
      ...
      {/* NeoForge shader guidance (only for shaders on NeoForge instances) */}
      {folder === "shaderpacks" && instanceLoader === "neoforge" && !showSearch && !disabled && (
        <div className="rounded-lg border border-sky-500/30 bg-sky-500/10 p-4">
          <div className="flex items-start gap-3">
            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="mt-0.5 shrink-0 text-sky-400">
              <circle cx="12" cy="12" r="10" /><line x1="12" y1="12" x2="12" y2="16" /><line x1="12" y1="8" x2="12.01" y2="8" />
            </svg>
            <div className="flex-1">
              <p className="text-sm font-medium text-sky-400">{t("content.neoforgeShadersTitle")}</p>
              <p className="mt-0.5 text-xs text-sky-400/70">
                {t("content.neoforgeShadersDesc")}
              </p>
            </div>
          </div>
        </div>
      ...
      {/* Iris not installed banner (only for shaders on Fabric/Vanilla) */}
      {folder === "shaderpacks" && instanceLoader !== "neoforge" && irisInstalled === false && !showSearch && !disabled && (
